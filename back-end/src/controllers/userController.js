...
class UserController {
  async registerUser(req, res) {
    try {
      ...
      const verificationToken = jwt.sign({ username }, secretKey, {
        expiresIn: "1d",
      });
  ...
  async verify(req, res) {
    ...
    try {
      ...
      console.log(decoded);
      const updatedUser =
        await userRepository.updateUserEmailVerificationStatus(
          decoded.username,
          true
        );
  ...
  async addToFavourite(req, res) {
    ...
    try {
      await userRepository.addNewFavoriteBook(userId, bookId);
      // Return the updated user object
      res.json({ message: "Book added to favourite successfully" });
    } catch (error) {
      console.error("Error updating account balance:", error);
      res.status(500).json({ error: "Unable to update account balance" });
    }

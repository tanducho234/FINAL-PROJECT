...
const MyBooksScreen = ({ navigation }) => {
  ...
  const fetchImageLinkAndUpdate = async (book) => {
    try {
      const response = await axios.get(`http://localhost:3000/image/getviewlink/${book.imagePath}`);
          ...
          <Pressable key={index} onPress={() =>
            ...
            <View style={styles.bookContainer}>
              ...
              <View style={{ marginRight: 10 }}>
                <Image source={{ uri: book.imageLink }} style={{ width: 100, height: 150,resizeMode: 'cover'}} />
                {/* Status below the image */}
                <Text style={{ color: book.status === 'Available' ? 'green' : 'red', marginTop: 5, marginLeft: 5 }}>
                  {book.status}
                </Text>
              </View>

...
import { View, Text, TextInput, FlatList, TouchableOpacity, ScrollView, Alert, StyleSheet, Pressable, Image } from 'react-native';
        ...
        },
        {
          ...
          onPress: (input) => {
            ...
            if (isNaN(amount) || amount % 10000 !== 0) {
              Alert.alert('Invalid Amount', 'Please enter a valid amount divisible by 10,000.');
            } else {
      ...
      <View style={styles.profileContainer}>
        ...
        <Text>Last transaction: 
          {transactions.length > 0 ?
            <Text style={{ color: transactions[0].type === 'Top-up' || transactions[0].type === 'Refund' ? 'green' : 'red' }}>
              {transactions[0].type === 'Top-up' || transactions[0].type === 'Refund' ? '+' : '-'}
              {transactions[0].amount.toLocaleString('vi-VN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} ₫
            </Text> :
            <Text>No transactions yet</Text>
          }
        </Text>
        <Text style={styles.balanceText}>{balance.toLocaleString('vi-VN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} ₫</Text>
            ...
            <View style={styles.transactionItem}>
              ...
              <View style={styles.settingRow}>
                <View>
                  ...
                  <Text>{new Date(item.transactionDate).toLocaleString('vi-VN')}</Text>
                </View>
                <Text style={{ color: item.type === 'Top-up' || item.type === 'Refund' ? 'green' : 'red' }}>
                {item.type === 'Top-up' || item.type === 'Refund' ? '+' : '-'}
                  {item.amount.toLocaleString('vi-VN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} ₫</Text>
              </View>
            </View>
          )}
        />
